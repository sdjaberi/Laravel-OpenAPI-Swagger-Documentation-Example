    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.languages.update", [$language->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $language->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('iso_code') ? 'is-invalid' : '' }}" type="text" name="iso_code" id="iso_code" value="{{ old('iso_code', $language->iso_code) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('active') ? 'is-invalid' : '' }}" type="checkbox" name="active" id="active"
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('text_direction') ? 'is-invalid' : '' }}" name="text_direction" id="text_direction" required>
                    @foreach($text_directions as $id => $text_direction)
                        <option value="{{ $text_direction }}" {{ old('active', $language->text_direction) == $text_direction ? 'selected' : '' }}>{{ $text_direction }}</option>
                    @endforeach
                </select>
